    ...
    <section className="py-16 md:py-24" aria-labelledby="why-heading">
      <div className="mx-auto max-w-container px-5 md:px-8">
        <div className="max-w-2xl mb-12">
          <h2 id="why-heading" className="text-3xl md:text-[2.5rem] font-semibold tracking-tight leading-[1.1] text-ink-900">
            Why teams come back.
          </h2>
        ...
        <div className="flex flex-col">
          {items.map((item, i) => (
            <div
              key={item.number}
              className={`flex gap-6 md:gap-10 py-8 md:py-10 border-t border-ink-100 ${
                i % 2 === 0 ? 'bg-white' : 'bg-ink-50'
              } -mx-5 px-5 md:-mx-8 md:px-8`}
            >
              <div className="shrink-0 w-10 md:w-16">
                <span className="font-mono text-3xl md:text-5xl font-semibold text-ink-100 select-none tabular-nums">
                  {item.number}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-xl md:text-2xl font-semibold tracking-tight text-ink-900">
                  {item.title}
                </h3>
                <p className="mt-3 text-base leading-relaxed text-ink-600 max-w-prose">
                  {item.body}
                </p>
              </div>
            </div>
          ))}
          <div className="border-t border-ink-100 -mx-5 md:-mx-8" />
        </div>
      </div>
    </section>
  );
